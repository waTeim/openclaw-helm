...
Generates make.env with the source (FROM) and target image settings
used by the Makefile build target.  Saves answers to build-config.json
so they become defaults on the next run.
...
def write_make_env(config: dict, output_dir: Path) -> Path:
    """Generate make.env file."""
    src = config["source"]
    tgt = config["target"]

    base_image = f'{src["registry"]}/{src["image"]}:{src["tag"]}'

    content = f"""\
# OpenClaw Kube - Makefile Configuration
# Generated by bin/configure-make.py
# Include this file in your Makefile with: -include make.env

# Source (FROM) image that the Dockerfile extends
BASE_IMAGE = {base_image}

# Target image coordinates
REGISTRY   = {tgt["registry"]}
IMAGE_NAME = {tgt["image"]}
IMAGE_TAG  = {tgt["tag"]}
"""

    output_path = output_dir / "make.env"
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("."),
                        help="Directory to write make.env and build-config.json (default: .)")
    ...
    write_make_env(config, args.output_dir)
